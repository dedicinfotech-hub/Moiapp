...
' . $tableRows . '
    ...
    <div class="footer">
        <p>Generated by Moi PassBook — Track gifts easily. Share with family.</p>
...
$body .= '<p style="color: #999; font-size: 11px;">Generated by Moi PassBook</p>';
$body .= '</body></html>';
...
if (!empty($mailConfig['smtp_host']) && class_exists('PHPMailer\PHPMailer\PHPMailer')) {
    // SMTP is configured and PHPMailer is available
    $mailSent = sendMail($to, $subject, $body, $event['creator']);
} elseif (!empty($mailConfig['smtp_host']) && !class_exists('PHPMailer\PHPMailer\PHPMailer')) {
    $mailError = 'SMTP configured but PHPMailer not installed. Run: composer require phpmailer/phpmailer';
    error_log('PDF Email Error: ' . $mailError);
} else {
    // Fallback to PHP mail() — works only if server has mail configured
    ...
    if (!$mailSent) {
        $mailError = 'PHP mail() returned false. Server may not have mail transport configured. Configure SMTP in .env for reliable delivery.';
        error_log('PDF Email Error: ' . $mailError . ' | To: ' . $to);
    }
}
