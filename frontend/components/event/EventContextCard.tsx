...
export default function EventContextCard({ event, icon = 'calendar', detailsHref }: EventContextCardProps) {
  ...
  return (
    <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4 mb-5 shadow-sm">
            ...
            <div>
              ...
              {isActive && (
                <div className="flex items-center gap-1.5 mt-1">
                  ...
                  <span className="text-[10px] font-semibold text-[#22C55E]">Function is Active</span>
                </div>
              )}
            </div>
            {detailsHref && (
              <Link href={detailsHref} className="text-[10px] font-semibold text-[#7C3AED] whitespace-nowrap shrink-0">
                View Details &gt;
              </Link>
            )}
          </div>
