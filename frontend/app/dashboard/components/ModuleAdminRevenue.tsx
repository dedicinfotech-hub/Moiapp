    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-lg text-tn-text">Revenue Management</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white border border-tn-border rounded-xl p-5 animate-pulse">
              <div className="h-4 bg-tn-border rounded mb-2 w-1/2"></div>
              <div className="h-8 bg-tn-border rounded mb-1 w-3/4"></div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-tn-border rounded-xl p-5">
          <p className="text-xs text-tn-muted mb-1">This Month</p>
          <p className="text-2xl font-bold text-tn-text">{formatCurrency(stats?.monthlyRevenue ?? 0)}</p>
          <p className="text-xs text-tn-muted mt-1">vs Last Month: {growth}%</p>
        </div>
        <div className="bg-white border border-tn-border rounded-xl p-5">
          <p className="text-xs text-tn-muted mb-1">Last Month</p>
          <p className="text-2xl font-bold text-tn-text">{formatCurrency(stats?.lastMonthRevenue ?? 0)}</p>
        </div>
        <div className="bg-white border border-tn-border rounded-xl p-5">
          <p className="text-xs text-tn-muted mb-1">Total Since Launch</p>
      ...
      {/* User Stats */}
      <div className="bg-white border border-tn-border rounded-xl p-5">
        <h3 className="font-semibold text-tn-text text-sm mb-4">User Statistics</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="text-center p-4 bg-tn-light rounded-lg">
            <p className="text-2xl font-bold text-tn-text">{stats?.totalUsers ?? 0}</p>
            <p className="text-xs text-tn-muted mt-1">Total Users</p>
          </div>
          <div className="text-center p-4 bg-tn-light rounded-lg">
            <p className="text-2xl font-bold text-tn-text">100%</p>
            <p className="text-xs text-tn-muted mt-1">Free Users (All users are free)</p>
          </div>
        </div>
      </div>
      ...
      {/* Recent Payments */}
      <div className="bg-white border border-tn-border rounded-xl p-5">
        <h3 className="font-semibold text-tn-text text-sm mb-4">Recent Payments</h3>
        ...
        <div className="mt-4 p-4 bg-tn-light border border-tn-yellow rounded-lg">
          <p className="text-xs text-tn-gold">
