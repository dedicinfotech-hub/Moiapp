...
export default function AllEntriesScreen() {
  ...
  return (
    <HostEntryShell slug="" title="All Moi Entries" activeTab="entries" onBack={() => router.push('/dashboard')} sidebarOverride={sidebarOverride}>
      ...
      <div className="mb-3">
        <label className="text-[10px] font-semibold text-tn-muted mb-1 block">Select Event</label>
        <select
          value={selectedEventId ?? ''}
          onChange={(e) => setSelectedEventId(Number(e.target.value))}
          className="w-full bg-white border border-tn-border rounded-xl px-3 py-2.5 text-xs"
        >
          {events.map((ev) => (
      ...
      <div className="grid grid-cols-4 gap-2 mb-5">
        {stats.map((s) => (
          <div key={s.label} className="bg-white border border-tn-border rounded-xl p-2 text-center">
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="flex-1 relative">
          ...
          />
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-tn-muted" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        </div>
        <select
          value={sort}
      ...
      <div className="space-y-0 bg-white border border-tn-border rounded-2xl overflow-hidden mb-24">
        {pageItems.length === 0 ? (
          <p className="py-10 text-center text-sm text-tn-muted">No entries found</p>
        ) : (
            ...
            <div key={entry.id} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? 'border-t border-tn-border' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1 mt-1 flex-wrap items-center">
                  <span className="text-[9px] font-medium bg-tn-green-soft text-tn-success px-1.5 py-0.5 rounded-full capitalize">{entry.payment_mode}</span>
                  <span className="text-[9px] font-medium bg-tn-blue-soft text-tn-yellow px-1.5 py-0.5 rounded-full capitalize inline-flex items-center gap-1">
                    {entry.gift_type === 'gift' ? <><Icon name="gift" size={12} /> Gift</> : entry.note?.includes('[Advance]') ? 'Advance' : 'Moi Collection'}
                  </span>
                  {entry.relation && (
                    <span className="text-[9px] font-medium bg-tn-yellow-light text-tn-gold px-1.5 py-0.5 rounded-full capitalize">{entry.relation}</span>
                  )}
                </div>
              </div>
              <div className="text-right shrink-0">
                ...
                <p className="text-[9px] text-tn-muted">
      ...
      <div className="fixed bottom-[68px] left-0 right-0 px-4 bg-gradient-to-t from-tn-light via-tn-light to-transparent pt-4 pb-3">
        <div className="max-w-lg mx-auto grid grid-cols-2 md:grid-cols-4 gap-2">
          <button type="button" onClick={() => { setSidebarOverride('closed'); router.push('/events/choose-type'); }} className="h-12 bg-tn-yellow text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-1">
            <span className="text-lg">+</span> Manual
          </button>
          <button type="button" onClick={() => { setSidebarOverride('closed'); router.push('/events/choose-type'); }} className="h-12 bg-tn-blue-soft text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-1">
          ...
          <button type="button" onClick={() => { setSidebarOverride('closed'); router.push('/events/choose-type'); }} className="h-12 bg-tn-gold text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-1">
          ...
          <button type="button" onClick={() => { setSidebarOverride('closed'); handleExportCSV(); }} className="h-12 border border-tn-border bg-white rounded-xl font-semibold text-xs text-tn-muted flex items-center justify-center gap-1">
