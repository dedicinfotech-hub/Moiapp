...
function DetailRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-tn-border last:border-0">
      <div className="w-8 h-8 rounded-lg bg-tn-light flex items-center justify-center shrink-0 text-tn-subtle">{icon}</div>
      <div className="flex-1 min-w-0">
        <p className="text-[10px] text-tn-subtle">{label}</p>
        <p className="text-sm font-semibold text-tn-text">{value}</p>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col px-6 py-6 overflow-y-auto">
        ...
        <div className="text-center mb-6">
          ...
          {isRejected && (
            <div className="inline-flex items-center gap-2 bg-tn-error/10 border border-tn-error/30 rounded-full px-4 py-2 mt-4">
              <span className="text-xs font-bold text-tn-error">Rejected</span>
            </div>
          )}
        </div>
        ...
        {isRejected && event.approval_reason && (
          <div className="bg-tn-error/10 border border-tn-error/30 rounded-xl p-4 mb-4">
            <p className="text-xs font-bold text-tn-error mb-1">Rejection Reason</p>
            <p className="text-xs text-tn-muted">{event.approval_reason}</p>
          </div>
        )}
